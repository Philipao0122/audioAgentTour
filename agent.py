...
# Define the Agent class locally to avoid circular imports
class Agent:
    ...
    async def run(self, input_text):
        """
        Execute the agent with the given input text using the OpenAI API.
        """
        import os
        ...
        try:
            ...
            # If output_type is defined, try to parse the response
            if hasattr(self, 'output_type') and self.output_type is not None:
                try:
                    ...
                    # Otherwise, pass the text as the 'output' field
                    return self.output_type(output=response_text)
                except (json.JSONDecodeError, TypeError) as e:
                    # If parsing fails, return the raw text in the output field
                    return self.output_type(output=response_text)
            
            # If no output_type is defined, return the raw text
            return {"output": response_text}
            
        except Exception as e:
            # Log the error and re-raise
            print(f"Error in {self.name} execution: {str(e)}")
            raise

class WebSearchTool:
    @staticmethod
    def search(query: str) -> str:
        # Mock implementation - in a real app, this would perform a web search
        return f"Search results for: {query}"

class ModelSettings:
    def __init__(self, tool_choice=None, **kwargs):
        ...
        # Update with any additional settings passed in
...
{
  "introduction": "Your engaging introduction here...",
  "architecture": "Architecture content here...",
  "history": "History content here...",
  "culture": "Culture content here...",
  "culinary": "Culinary content here...",
  "conclusion": "Your thoughtful conclusion here..."
}

Important Notes:
- The JSON object must be valid and contain all six fields.
- Each field should contain the corresponding section of the tour.
